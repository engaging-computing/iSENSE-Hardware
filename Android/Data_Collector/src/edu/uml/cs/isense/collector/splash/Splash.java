...
public class Splash extends TabActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * // Apply custom font to "iSENSE Data Collector" appName =
		 * (FontFitTextView) findViewById(R.id.title_text); final Typeface tf1 =
		 * Typeface .createFromAsset(getAssets(), "facets.otf");
		 * appName.setTypeface(tf1);
		 * 
		 * // Begin animation of abstract_circle final RelativeLayout
		 * abstractCircle = (RelativeLayout) findViewById(R.id.abstract_layout);
		 * final Animation rotate = AnimationUtils.loadAnimation(this,
		 * R.anim.fastspinner); abstractCircle.startAnimation(rotate);
		 * 
		 * // Set "EULA" in the CheckBox's string to become clickable eulaCheck
		 * = (CheckBox) findViewById(R.id.eula_check);
		 * 
		 ...
		 * Spannable spans = (Spannable) eulaText.getText(); ClickableSpan
		 * clickSpan = new ClickableSpan() {
		 * 
		 * @Override public void onClick(View v) {
		 * 
		 * PackageInfo versionInfo = getPackageInfo(); Intent iEula = new
		 * Intent(mContext, Eula.class); iEula.putExtra("versionName",
		 * versionInfo.versionName); startActivityForResult(iEula,
		 * EULA_REQUESTED); } }; spans.setSpan(clickSpan, spans.length() - 5,
		 * spans.length() - 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
		 * 
		 * // Set listeners for the "Continue" and "Exit" buttons final Button
		 * cont = (Button) findViewById(R.id.splash_continue);
		 * cont.setOnClickListener(new OnClickListener() {
		 * 
		 * @Override public void onClick(View v) { if (eulaCheck.isChecked()) {
		 * setResult(RESULT_OK); finish(); } else
		 * w.make("You must accept the EULA to continue!", Toast.LENGTH_LONG,
		 * "x"); } });
		 * 
		 * final Button exit = (Button) findViewById(R.id.splash_exit);
		 * exit.setOnClickListener(new OnClickListener() {
		 ...
		 * 
		 * @Override public void onClick(View v) { Intent iAppGuide = new
		 * Intent(Splash.this, ApplicationGuide.class);
	...
	/*
	 * // Used by EULA private PackageInfo getPackageInfo() { PackageInfo pi =
	 * null; try { pi = mContext.getPackageManager().getPackageInfo(
	 * mContext.getPackageName(), PackageManager.GET_ACTIVITIES); } catch
	 * (PackageManager.NameNotFoundException e) { e.printStackTrace(); } return
	 * pi; }
	 * 
	 * // Method to check the EULA CheckBox off if the user has accepted the
	 * EULA protected void onActivityResult(int requestCode, int resultCode,
	 * Intent data) { super.onActivityResult(requestCode, resultCode, data);
	 * 
	 * if (requestCode == EULA_REQUESTED) { if (resultCode == RESULT_OK) {
	 * eulaCheck.setChecked(true); } else if (resultCode == RESULT_CANCELED) {
	 * eulaCheck.setChecked(false); } } }
	 * 
	 * @Override public void onBackPressed() { setResult(RESULT_CANCELED);
	 * finish(); }
	 * 
	 ...
	 * super.onConfigurationChanged(newConfig); onCreate(null); }
	 */
